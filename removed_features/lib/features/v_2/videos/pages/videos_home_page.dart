...
class VideosHomePage extends StatefulWidget {
  ...
  const VideosHomePage({super.key, required this.type});
...
class _VideosHomePageState extends State<VideosHomePage> {
  ...
  Future<void> loadPageData({bool forceRefresh = false}) async {
    await loadPlaylists();
  ...
  Future<void> loadPodcastPlaylistThumbnails() async {
    ...
    final results = await Future.wait(
      playlists.map((playlist) async {
        ...
        return MapEntry(playlist.playlistId, playlistVideos.first.thumbnailUrl);
      }),
    );
  ...
  Future<void> loadMergedVideos({bool forceRefresh = false}) async {
    if (!mounted) return;
  ...
  void openPlaylist(BuildContext context, YoutubePlaylistModel playlist) {
    Navigator.push(
      context,
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) =>
            PlaylistVideosPage(playlist: playlist),
  ...
  Future<void> openVideo(YoutubeVideoModel video) async {
    ...
    await Navigator.push(
      ...
      PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) =>
            YoutubeVideoPlayerPage(video: video, playlistId: 'merged_videos'),
        transitionDuration: Duration.zero,
        reverseTransitionDuration: Duration.zero,
      ),
    );
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            children: [
              VideoPageHeader(title: widget.type.arabicTitle),
              SizedBox(height: 8.h),
              VideoCacheNotice(visible: isUsingCacheFallback),
              Expanded(
                child: isVideosPage
                    ? _VideosFeedBody(
                        isLoading: isLoading,
    ...
    if (playlists.isEmpty) {
      return RefreshIndicator(
        ...
        child: const SingleChildScrollView(
          ...
          child: SizedBox(
            height: 350,
            child: _EmptyVideosMessage(text: 'لا توجد قوائم بودكاست حاليًا'),
          ),
        ),
      );
    }
            ...
            children: [
              ...
              Expanded(
                child: _YoutubeActionButton(
                  ...
                  leading: const _YoutubeLogoMark(compact: true),
                ),
              ),
            ],
          ),
...
class _YoutubeLogoMark extends StatelessWidget {
  ...
  const _YoutubeLogoMark({this.compact = false});
...
class _EmptyVideosMessage extends StatelessWidget {
  ...
  const _EmptyVideosMessage({required this.text});
