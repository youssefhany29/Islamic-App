...
def main() -> None:
    ...
    for index, page in enumerate(pages, start=1):
        ...
        total_words += int(pack["totalWords"])
        total_missing_words += int(pack["missingWords"])
        total_missing_glyphs += int(pack["missingGlyphs"])
        ...
        report_pages.append(
            {
                "page": page,
                "words": pack["totalWords"],
                "chars": pack["totalChars"],
                "svgDocs": len(pack["svgDocs"]),
                "missingWords": pack["missingWords"],
                "missingGlyphs": pack["missingGlyphs"],
                "gzipBytes": page_size,
            }
        )
        ...
        print(
            ...
            f"words={pack['totalWords']} | "
            f"svg={len(pack['svgDocs'])} | "
            f"missingGlyphs={pack['missingGlyphs']} | "
    ...
    report_path = out_dir / "qpc_svg_pages_report.json"
