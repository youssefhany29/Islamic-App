...
class HadithTodayReviewPage extends StatefulWidget {
  const HadithTodayReviewPage({super.key});

  @override
  State<HadithTodayReviewPage> createState() => _HadithTodayReviewPageState();
}

class _HadithTodayReviewPageState extends State<HadithTodayReviewPage> {
  final HadithMemoryProgressService _memoryService =
      const HadithMemoryProgressService();
  final HadithDataService _dataService = const HadithDataService();

  late Future<List<HadithMemoryItemStateModel>> _reviewsFuture;

  List<HadithMemoryItemStateModel> _reviews = [];
  int _currentIndex = 0;
  ...
  Future<List<HadithMemoryItemStateModel>> _loadReviews() async {
    final reviews = await _memoryService.getDueReviews();
    return reviews;
  }

  Future<HadithItemModel?> _findItem(HadithMemoryItemStateModel state) async {
  ...
  Future<void> _submitRating(
    ...
    HadithMemoryItemStateModel state,
  ) async {
    if (_isSaving) return;
    ...
    await _memoryService.saveAttempt(
      HadithMemoryAttemptModel(
        ...
        repetitionCount: 1,
        practiceMode: HadithPracticeMode.test,
      ),
    );
  ...
  void _showRatingSnackBar(HadithMemoryRating rating) {
    final bool large = _todayReviewLargeScreen(context);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: FutureBuilder<List<HadithMemoryItemStateModel>>(
          ...
          builder: (context, snapshot) {
            ...
            return FutureBuilder<HadithItemModel?>(
                              ...
                              if (_showAnswer)
                                _RatingActionsCard(
                                  ...
                                  onMastered: () => _submitRating(
                                    HadithMemoryRating.mastered,
                                    state,
                                  ),
                                  onPartial: () => _submitRating(
                                    HadithMemoryRating.partial,
                                    state,
                                  ),
                                  onReview: () => _submitRating(
                                    HadithMemoryRating.review,
                                    state,
                                  ),
                                )
                              else
...
class _ProgressHeaderCard extends StatelessWidget {
  ...
  final int current;
  final int total;
  final HadithMemoryItemStateModel state;
...
class _ReviewQuestionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final _TodayReviewMetrics m = _TodayReviewMetrics.of(context);
    final String displayText = item?.text ?? 'لم يتم العثور على نص هذا الحديث.';
...
class _ShowAnswerHintCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: _ArabicText(
        'بعد ما تحاول تسترجع الحديث، اضغط إظهار الإجابة ثم قيّم حفظك.',
        fontSize: m.bodySize,
        fontWeight: FontWeight.w600,
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    'لو عندك مراجعات قادمة، هتظهر في تقويم المراجعة داخل صفحة الأحاديث.',
                    textAlign: TextAlign.center,
                    textDirection: TextDirection.rtl,
                    style: AppTextStyles.caption(context).copyWith(
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                children: [
                  ...
                  SizedBox(height: m.large ? 8 : 6.h),
                  Text(
                    'راجعت $reviewedCount أحاديث. تم تحديث خطة الحفظ والمواعيد القادمة تلقائيًا.',
                    textAlign: TextAlign.center,
                    textDirection: TextDirection.rtl,
                    style: AppTextStyles.caption(context).copyWith(
                      fontWeight: FontWeight.w500,
