    ...
    return SizedBox(
      ...
      child: Column(
        children: const [
          ...
          _VideoVerticalGap(),
          AppVideoTile(type: AppVideoTileType.lessons, isWide: true),
        ],
      ),
    );
  }
...
class AppVideoTile extends StatelessWidget {
  const AppVideoTile({super.key, required this.type, required this.isWide});
  ...
  void _showPodcastsComingSoon(BuildContext context) {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: VideoComponents(
        onTap: () {
          switch (type) {
            ...
            case AppVideoTileType.podcasts:
              _showPodcastsComingSoon(context);
              break;
        ...
        imageHeight: 72.h,
        category: Category(image: _image, text: _text),
      ),
    );
  }
...
enum AppVideoTileType { recitations, podcasts, lessons }
