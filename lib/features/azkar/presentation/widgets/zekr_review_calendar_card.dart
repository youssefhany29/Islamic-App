...
part 'zekr_review_calendar_widgets.dart';

bool _zekrCalendarLargeScreen(BuildContext context) {
  final Size size = MediaQuery.sizeOf(context);
  return size.shortestSide >= 600 || (size.width >= 700 && size.height >= 500);
}

class ZekrReviewCalendarCard extends StatefulWidget {
  const ZekrReviewCalendarCard({
    super.key,
  ...
  @override
  State<ZekrReviewCalendarCard> createState() => _ZekrReviewCalendarCardState();
}

class _ZekrReviewCalendarCardState extends State<ZekrReviewCalendarCard> {
  final ZekrMemoryProgressService _service = const ZekrMemoryProgressService();

  late Future<List<ZekrMemoryItemStateModel>> _statesFuture;
  ...
  @override
  void didUpdateWidget(covariant ZekrReviewCalendarCard oldWidget) {
  ...
  List<ZekrMemoryItemStateModel> _itemsForDate(
  ...
  void _showDayReviewsSheet({
    ...
    required List<ZekrMemoryItemStateModel> items,
  }) {
    AppHaptics.tap(context);

    final theme = Theme.of(context);
    final bool isToday = _isToday(date);
    final bool isLargeScreen = _zekrCalendarLargeScreen(context);
    ...
    final String sheetSubtitle = items.isEmpty
        ? 'لا يوجد عليك أي ذكر للمراجعة في هذا اليوم.'
        : 'عليك ${items.length} ${items.length == 1 ? 'ذكر' : 'أذكار'} للمراجعة. ابدأ بالأضعف لتثبيت الحفظ.';
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<ZekrMemoryItemStateModel>>(
