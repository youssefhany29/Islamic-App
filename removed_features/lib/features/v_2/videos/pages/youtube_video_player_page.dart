...
class _YoutubeVideoPlayerPageState extends State<YoutubeVideoPlayerPage> {
  ...
  Future<void> _initPlayer() async {
    ...
    final savedPosition = await VideoProgressStorage.getVideoPositionSeconds(
      widget.video.id,
    );
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          ...
          child: currentController == null || isInitializing
              ? _InitialLoadingView(textColor: textColor, onBack: _goBack)
                                  ...
                                  Expanded(
                                    child: _PlayerActionButton(
                                      icon: preferHd
                                          ? Icons.hd_rounded
                                          : Icons.sd_rounded,
                                      label: preferHd ? 'HD مفعل' : 'جودة أسرع',
...
class _InitialLoadingView extends StatelessWidget {
  ...
  const _InitialLoadingView({required this.textColor, required this.onBack});

  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        _PlayerHeader(title: 'المشاهدة', textColor: textColor, onBack: onBack),
        const Expanded(
          child: Center(
            child: CircularProgressIndicator(color: Color(0xff21C58E)),
          ),
        ),
      ],
    );
...
class _PlayerHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(right: 14.w, left: 14.w, top: 14.h),
      child: Row(
        children: [
          IconButton(
          ...
          SizedBox(width: 38.w, height: 38.h),
        ],
      ),
    );
  }
...
class _YoutubeIconOnlyButton extends StatelessWidget {
  ...
  const _YoutubeIconOnlyButton({required this.onTap});
