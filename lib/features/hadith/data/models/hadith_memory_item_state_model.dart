import 'hadith_memory_attempt_model.dart';

enum HadithMemoryLevel { fresh, needsReview, stabilizing, memorized, strong }

extension HadithMemoryLevelX on HadithMemoryLevel {
  String get label {
    switch (this) {
      case HadithMemoryLevel.fresh:
        return 'جديد';
      case HadithMemoryLevel.needsReview:
        return 'يحتاج مراجعة';
      case HadithMemoryLevel.stabilizing:
        return 'قيد التثبيت';
      case HadithMemoryLevel.memorized:
        return 'محفوظ';
      case HadithMemoryLevel.strong:
        return 'محفوظ بثبات';
    }
  }
}

class HadithMemoryItemStateModel {
  const HadithMemoryItemStateModel({
  ...
  final HadithMemoryRating lastRating;
  final DateTime lastReviewedAt;
  final DateTime nextReviewAt;
  final HadithMemoryLevel level;
  ...
  HadithMemoryItemStateModel copyWith({
    String? itemId,
    String? categoryId,
    String? itemTitle,
    ...
    int? currentStreak,
    int? bestStreak,
    HadithMemoryRating? lastRating,
    DateTime? lastReviewedAt,
    DateTime? nextReviewAt,
    HadithMemoryLevel? level,
    DateTime? createdAt,
    DateTime? updatedAt,
  }) {
  ...
  factory HadithMemoryItemStateModel.fromJson(Map<String, dynamic> json) {
    final rawRating =
        json['lastRating'] as String? ?? HadithMemoryRating.review.name;
    final rawLevel = json['level'] as String? ?? HadithMemoryLevel.fresh.name;
    ...
    return HadithMemoryItemStateModel(
      ...
      lastRating: HadithMemoryRating.values.firstWhere(
      ...
      level: HadithMemoryLevel.values.firstWhere(
        (item) => item.name == rawLevel,
        orElse: () => HadithMemoryLevel.fresh,
      ),
